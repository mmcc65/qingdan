...
- 单次提醒和循环提醒调度
- 重启手机后恢复提醒
- 桌面小组件最小尺寸为 2×2，可在待办、重复、项目、日程和随笔之间切换，并向下滚动查看全部内容；点击任务行圆圈可直接完成
- 使用与电脑端一致的“清”字图标
- 0.5.0 起支持应用内自动检查、下载、校验并安装正式更新

## 发布手机更新

维护者可在 `apps/web/src/release-config.js` 中填写 GitHub Releases 或其他可信 HTTPS 更新清单。需要自建 Supabase 发布通道时，在 SQL Editor 运行 `infra/supabase/migrations/002_mobile_releases.sql`，创建公开读取的 `qingdan-releases` 存储桶。
