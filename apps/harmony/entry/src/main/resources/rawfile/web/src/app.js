...
const REPEAT_RULES = ["每天", "每个工作日", "每周", "每月", "每隔 2 周"];
let reorderGesture = null;
let suppressReorderClick = false;
let harmonyPort = null;

restoreStateFromNative();
...
function mergeNativeState(raw) {
  ...
  } catch { /* Ignore invalid native state and keep the browser copy. */ }
  return false;
}

function restoreStateFromNative() {
  try {
    mergeNativeState(window.QingdanAndroid?.getState?.());
...
function syncDesktopState() {
  ...
  if (harmonyPort) harmonyPort.postMessage(JSON.stringify({ type: "state", state }));
}
...
function taskCard(task, context = "root", projectId = "") {
  ...
  const repeatRule = repeatRuleSummary(task);
  if (repeatRule) meta.push(`<span>${escapeHtml(repeatRule)}</span>`);
...
function normalizeRepeatSchedules(task) {
  const schedules = Array.isArray(task?.repeatSchedules) && task.repeatSchedules.length
    ? task.repeatSchedules
    : [{ rule: task?.rule || "每周", time: "" }];
  return schedules.map(schedule => ({
    rule: REPEAT_RULES.includes(schedule?.rule) ? schedule.rule : "每周",
    time: /^\d{2}:\d{2}$/.test(schedule?.time || "") ? schedule.time : ""
  }));
}

function repeatRuleSummary(task) {
  if (task?.kind !== "repeat") return task?.rule || "";
  return normalizeRepeatSchedules(task)
    .map(schedule => `${schedule.rule}${schedule.time ? ` ${schedule.time}` : ""}`)
    .join("、");
}

function renderRepeatSchedules(schedules = []) {
  const values = schedules.length ? schedules : [{ rule: "每周", time: "" }];
  $("#repeat-schedules").innerHTML = values.map((schedule, index) => `<div class="repeat-schedule" data-schedule-index="${index}">
    <div class="reminder-slot-heading"><strong>重复时间 ${index + 1}</strong>${index ? '<button type="button" class="text-button remove-repeat-schedule" data-schedule-index="' + index + '">删除</button>' : ""}</div>
    <div class="two-columns">
      <label class="field"><span>规则</span><select data-repeat-field="rule">${REPEAT_RULES.map(rule => `<option value="${rule}" ${schedule.rule === rule ? "selected" : ""}>${rule}</option>`).join("")}</select></label>
      <label class="field"><span>执行时间 <em>可选</em></span><input type="time" data-repeat-field="time" value="${escapeHtml(schedule.time || "")}"></label>
    </div>
  </div>`).join("");
}

function readRepeatSchedules() {
  const seen = new Set();
  return $$("#repeat-schedules .repeat-schedule").map(schedule => ({
    rule: schedule.querySelector('[data-repeat-field="rule"]').value,
    time: schedule.querySelector('[data-repeat-field="time"]').value
  })).filter(schedule => {
    const key = `${schedule.rule}|${schedule.time}`;
...
function openTaskDialog(kind, task = null, projectId = "", priority = "normal") {
  ...
  renderRepeatSchedules(kind === "repeat" ? normalizeRepeatSchedules(task) : []);
...
$("#task-form").addEventListener("submit", event => {
  ...
  if (kind === "repeat") {
    task.repeatSchedules = readRepeatSchedules();
    task.rule = repeatRuleSummary(task);
  }
...
$("#add-repeat-schedule").addEventListener("click", () => {
  renderRepeatSchedules([...readRepeatSchedules(), { rule: "每天", time: "" }]);
});
$("#repeat-schedules").addEventListener("click", event => {
  const remove = event.target.closest(".remove-repeat-schedule");
  if (!remove) return;
  const schedules = readRepeatSchedules();
  schedules.splice(Number(remove.dataset.scheduleIndex), 1);
  renderRepeatSchedules(schedules);
});
...
window.addEventListener("message", event => {
  if (event.data !== "__qingdan_harmony_port__") return;
  const port = event.ports?.[0];
  if (!port) return;
  harmonyPort = port;
  harmonyPort.onmessage = messageEvent => {
    try {
      const message = JSON.parse(messageEvent.data);
      if (message?.type !== "state" || typeof message.stateJson !== "string") return;
      if (mergeNativeState(message.stateJson)) {
        render();
        toast("已从鸿蒙本机数据更新");
      }
      syncDesktopState();
  ...
  harmonyPort.postMessage(JSON.stringify({ type: "ready" }));
});
