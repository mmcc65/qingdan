...
- **按使用场景拆分**：一次性待办、重复任务、日程和项目各自拥有清晰入口，避免所有能力挤在一张平面列表里。
...
- 使用日期选择器或前后一天按钮查看每天的安排。
- 行程按开始时间排列，可完成、取消、恢复或删除。
- 支持与待办一致的单次提醒和多时间段循环提醒；手机端可使用系统精确闹钟。
- 全局搜索也会包含进行中的日程。
...
- 当前 `0.7.0` 以每个用户的一份完整状态进行同步，较新的时间戳覆盖较旧状态；复杂的并发字段合并仍是后续工作。
