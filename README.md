...
| 管理每天或每周重复的事情 | 每天、工作日、每周、每月、每两周等规则；一个任务可添加多个规则和执行时间；完成本次或跳过本次，不影响后续计划 |
| 推进复杂目标 | 建立大项目和多级子项目，在每层添加任务；项目可独立完成，不会强制改变内部任务状态 |
| 安排当天及未来行程 | 日程总览按日期集中显示今天及未来未完成行程；顶部只提示今天的数量，三个状态按所选日期展示 |
| 随手记录小事情 | 独立随笔模块；自动编号、备注、置顶和手动排序，不要求把每件事都变成任务 |
| 防止忘记重要事情 | 单次提醒或多时间段循环提醒；Windows 原生提醒窗口、Android 精确闹钟、重启后自动恢复 |
| 不打开应用快速查看 | Android 圆角桌面组件最小 2×2，可滚动查看全部内容，顶部按钮循环切换待办、重复、项目、今日日程和随笔 |
...
- **按使用场景拆分**：待办、重复、项目、日程、随笔各自拥有清晰入口，避免所有能力挤在一张平面列表里。
...
| 分层任务模型 | 待办、重复任务、多级项目、项目内任务与日程相互独立 | 临时事项、习惯、长期目标和当天行程不会混成一团 |
...
- 置顶任务优先显示；其余任务按照手动顺序排列，也可以从任务菜单中上移或下移。
- 节点会显示“今天、明天、具体日期”等易读时间，过期节点会单独标识。
- 可将节点一键延后到明天、后天或下周，不用重新选择日期。
- 支持完成、取消和直接删除；已完成待办仍按重要、普通、随后分栏，取消记录独立展示。
...
- 内置每天、每个工作日、每周、每月和每隔两周等重复规则；同一任务可添加多条规则与执行时间。
- 每个重复任务同样支持优先级、节点、备注、置顶、手动排序和提醒。
- 循环提醒支持在一个任务中添加多个独立的提醒时间段。
- “完成本次”只生成一条完成记录，原重复计划继续保留。
- “跳过本次”只生成一条跳过记录，不影响下一次计划。
- 完成记录与跳过记录分别归档，可单独永久删除。

> 当前版本可为一个任务添加多条重复规则和执行时间；按星期几、每月第几天等复杂自定义日历规则仍在后续计划中。
...
- 项目可独立完成，不会改动其子项目或任务的状态；已完成项目集中显示在“已完成项目”中。
- 点击项目名称可进入详情，完整查看其中的任务和子项目；项目频道的数量只统计顶层大项目。
- 支持折叠项目、置顶项目、手动调整项目顺序、重命名，以及继续添加子项目。
- 已完成的项目任务默认隐藏在所属项目底部，可按需展开、收起或恢复，不会混入项目完成列表。
...
- Windows 与手机端可提供原生提醒，Android/HarmonyOS 可使用系统精确闹钟。
- 全局搜索也会包含进行中的日程。

### 5. 随笔

随笔用于记录不需要完成、取消或提醒的小事情。

- 每篇随笔自动显示序号，并可添加独立备注。
- 支持置顶和取消置顶；置顶组与普通组内均可单独调整顺序。
- 可使用“...”菜单上移、下移，也可直接长按卡片后拖动排序。
- 全局搜索同时覆盖随笔正文和备注，手机桌面组件也可切换查看随笔。
...
- **循环提醒**：可添加多个提醒时间段，每段分别设置开始时间、结束时间、间隔数值，以及分钟/小时/天单位。
...
- 待办和项目任务分为进行中、已完成、已取消三种状态；项目本身也可独立完成或取消。
- 重复任务保留独立的完成本次与跳过本次历史，不会混入普通待办归档。
- 任务卡可以直接完成、编辑、延后、取消或删除。
- 进行中的任务、项目和随笔均可长按卡片拖动排序。
- 全局搜索覆盖进行中的待办、重复任务、项目、项目任务、日程及备注。
...
- 当前 `0.11.0` 以每个用户的一份完整状态进行同步，较新的时间戳覆盖较旧状态；复杂的并发字段合并仍是后续工作。
...
### 11. Android / HarmonyOS 手机端

- 复用完整的待办、重复、项目、搜索、同步和设置界面。
- 使用 Android 系统闹钟与通知通道提供后台通知和横幅提醒。
- 设备重启、系统时间变化或应用升级后自动恢复提醒计划。
- 桌面小组件最小为紧凑的 2×2；顶部标题行使用左右按钮，在待办、重复、顶层项目、今日日程和随笔之间双向循环，标题始终居中且不会挤占下方列表。
- 组件会记住当前分类，列表可向下滚动查看全部内容；点击卡片可进入应用，点击任务行圆圈可完成待办、完成一次重复任务、结束项目或完成日程。
- 支持应用内检查、下载、SHA-256 校验并调起系统安装更新。
- 除可安装 Android APK 的 HarmonyOS 外，仓库现包含 HarmonyOS NEXT 原生客户端与服务卡片工程；原生 HAP 需使用 DevEco Studio 配置签名后构建。
...
| HarmonyOS（Android 兼容） | 已实机验证 | 华为 Pura 70 Pro / HarmonyOS 4.2 兼容 Android APK |
| HarmonyOS NEXT | 原生工程已提供 | ArkWeb 复用界面，Form Kit 服务卡片；需使用 DevEco Studio 签名构建 HAP |

> Android APK 与 HarmonyOS NEXT 原生 HAP 是两条独立安装路线；GitHub Release 当前提供前者，后者请按 `apps/harmony/README.md` 构建。
