...
它把不同类型的事情分开管理：临时事项放进待办，周期事务放进重复任务，复杂目标拆成多级项目，当天行程安排在日程中。任务可以设置节点、备注、优先级和提醒；Windows 与手机端负责系统级提醒，Android 桌面组件还能直接查看和完成待办。
...
- 桌面小组件最小为紧凑的 2×2；会随组件高度显示最多七项待办，点击卡片可进入应用，点击每行圆圈可直接完成。
