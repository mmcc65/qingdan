...
它把不同类型的事情分开管理：临时事项放进待办，周期事务放进重复任务，复杂目标拆成多级项目，当天行程安排在日程中。任务可以设置节点、备注、优先级和提醒；Windows 与手机端负责系统级提醒，Android 桌面组件还能直接切换查看和完成各类事项。
...
| 不打开应用快速查看 | Android 圆角桌面组件最小 2×2，顶部按钮循环切换待办、重复、项目和今日日程；点击打开应用或直接完成 |
...
- **按使用场景拆分**：待办、重复、项目、日程各自拥有清晰入口，避免所有能力挤在一张平面列表里。
...
### 10. Android / HarmonyOS 手机端

- 复用完整的待办、重复、项目、搜索、同步和设置界面。
- 使用 Android 系统闹钟与通知通道提供后台通知和横幅提醒。
- 设备重启、系统时间变化或应用升级后自动恢复提醒计划。
- 桌面小组件最小为紧凑的 2×2；顶部标题行使用左右按钮，在待办、重复、顶层项目和今日日程之间双向循环，标题始终居中且不会挤占下方列表。
- 组件会记住当前分类，并随高度显示最多七项；点击卡片可进入应用，点击每行圆圈可完成待办、完成一次重复任务、结束项目或完成日程。
